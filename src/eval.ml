...
let rec eval_exp env = function
  ...
  (* 関数適用式 *)
  | DFunExp (id, exp) -> DProcV (id, exp) (*3.4.5 dfunのための関数適用*)
  | AppExp (exp1, exp2) ->
      (* 関数 exp1 を現在の環境で評価 *)
      ...
      (match funval with
          ...
          |DProcV (id, body) -> (*3.4.5 dfunの評価*)
            let newenv = Environment.extend id arg env in
            eval_exp newenv body
          | _ -> 
            (* 評価結果がクロージャでなければ，実行時型エラー *)
            err ("Non-function value is applied"))
  | EXPlusExpr -> (*3.4.2 (+)の適用式*)
    let n = Var "n" in
    let m = Var "m" in
    let f = FunExp("m", BinOp(Plus, n, m)) in
    ProcV("n", f, ref env)
  | EXMultExpr -> (*3.4.2 ( * )の適用式*)
    let n = Var "n" in
    let m = Var "m" in
    let f = FunExp("m", BinOp(Mult, n, m)) in
    ProcV("n", f, ref env)
